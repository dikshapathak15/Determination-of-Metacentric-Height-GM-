document.querySelector("#moveButton").addEventListener("click", () => {
  moveWeightAndRotate(410); // Calls the function when the button is clicked
});
...
function moveWeightAndRotate(targetPosition, delay = 100) {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      weight.style.transform = "translate(0px, 28px) rotate(110deg)";
      pontoon.style.transform = "translate(-468px, -100px) rotate(20deg)";

      // Hide the moveButton after all processes
      setTimeout(() => {
        moveButton.style.display = "none"; // Hides the button
        instruction.textContent =
          "STEP 6:Click on the protractor and measure the angle of tilt (θ) and the distance(x) using it."; // Changes the h1 text

const weight = document.getElementById('weight');
const moveButton = document.getElementById('moveButton');


    function animateImage() {
      setTimeout(() => {
        // Move upward
        weight.style.top = '50px';
      }, 500);

      setTimeout(() => {
        // Move left
        weight.style.left = '454px';
      }, 1500);

      setTimeout(() => {
        // Move downward
        weight.style.top = '93px';
      }, 2500);

      setTimeout(() => {
        // Rotate 90 degrees
        weight.style.transform = 'rotate(90deg)';
      }, 3500);

      setTimeout(() => {
        // Move left again
        weight.style.left = '360px';

        // Make the image fixed and unclickable
        weight.classList.add('fixed');
        weight.removeEventListener('click', handleClick);

           // Change the instruction text after completion
        setTimeout(() => {
          ins.textContent = "STEP 5:Click on the move button to move the weight horizontally across a cross bar towards one side (X), causing it to tilt.";

           setTimeout(() => {
            // Display the move button
        moveButton.style.display = 'block';
        }, 200); 
      }, 1200); 
      
      }, 4500);
    }

    // Click handler function
    function handleClick() {
      animateImage();
    }

    // Initialize starting position
    weight.style.top = '265px';
    weight.style.left = '637px';

    // Add click event listener
    weight.addEventListener('click', handleClick);
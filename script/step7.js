  ...
  const instruction = document.getElementById('ins');
        ...
        // Initial position
        const initialTop = 239;
        const initialLeft = 726;

        function moveUp() {
            protractor.style.top = `${initialTop - 227}px`;
        }

        function moveLeft() {
            protractor.style.left = `${initialLeft - 420}px`;
        }
        ...
        function moveBack() {
            ...
            protractor.style.left = `${initialLeft}px`;
              setTimeout(() => {
                instruction.textContent = "STEP 7: Calculate the metancentric height(GM)";
                fadeOutImages();
